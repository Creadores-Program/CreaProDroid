...
public class MaxIaManager{
    ...
    public String promptMax(String prompt){
      String promptToMax = clearPalabra(prompt.split("\\[File:")[0].replaceAll(" " + this.UserName + " ", "%UserName")).toLowerCase().replaceAll("[\\p{Z}\\s]+", " ").trim().replaceAll("creaprodroid", "%BotName").replaceAll("[^a-zA-Z]", "");
      try{
